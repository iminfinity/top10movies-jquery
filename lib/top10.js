...
function makeModal(i,j){
	...
	synopsis = "<span class=\"labels-left\">Synopsis </span>" + synopsis;
...
function createGraphic(data){
	...
	for(let i = 2000; i < 2011; i++){
		for(let j = 1; j < 11; j++){
			...
			// movieImg = "<img class=\"image-main\" src=\""+ img +"\" />";
			// $td.html(movieImg)
			...
			$td.on("mouseover", function(event){ 
				// imdb = mainarr[i.toString()][j.toString()].imdbrating;
				xxx = "<a href=\"#modal-id\"  id=\"click-modal\" rel=\"modal:open\" title=\"Details\"></a>"
				$(event.target).html(xxx);
